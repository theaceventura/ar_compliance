...
def _upsert_threat(obj) -> bool:
    ...
    if row:
        cur.execute(
            ...
            SET item_type=?, title=?, summary=?, link=?, published_at=?, severity=?, kev_flag=?, products_text=?, raw_payload=?
            WHERE id=?
            """,
            (*payload, row["id"]),
        )
...
def _parse_vulnerabilities(cvrf_data: Dict, release_meta: Dict, threat_cls) -> List:
...
def import_msrc_threats() -> int:
    ...
    for idx, header in enumerate(items):
        ...
        if not cvrf:
            continue
        try:
            threats = _parse_vulnerabilities(cvrf, release_meta, ThreatObject)
        except Exception as exc:
        ...
        for t in threats:
            try:
                ...
                threat_ingestion._set_status("msrc", error=f"Failed to upsert {t.cve_id or 'unknown'}: {exc}", progress=int((idx / total) * 90))
                continue
    threat_ingestion._set_status("msrc", message="Completed", progress=100, error=None)
    return inserted
